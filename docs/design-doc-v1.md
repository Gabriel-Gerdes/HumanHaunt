...
5. Phase boosts apply through `current_points` before scoring: each carried-forward unclaimed task gains the phase-start payload's `boostUnclaimedBy` exactly once per phase-start event encountered in canonical event-chain order during replay, counted idempotently by event UID — never per wall-clock elapsed phase. A task unclaimed across N phase-start events present in the local chain is therefore worth base points plus N times the boost. Partial replays apply boosts only for phase-start events already imported into the local chain, so devices with shorter chain prefixes show proportionally boosted values until their chains converge.
...
Rotation under out-of-order mesh delivery: a device can encounter admin events tagged with the new secret before the `admin_secret_rotated` package carrying it arrives, and lagging devices keep emitting tags under the previous secret. Recovery rules: after importing a rotation event, a device MUST retain the immediately previous `adminSecret` alongside the new one for a grace period spanning the overlap window, and verify every incoming `adminAuth` tag against both `{current, previous}` secrets, accepting a match against either. When an admin event fails verification against all locally held secrets, the device MUST still reject that event (never store or relay it) but MUST NOT hard-drop, ban, or disconnect the sender, because the failure usually means this node is behind on rotation rather than under attack. Instead the device initiates a normal `hello` / `request_events` round so the sender delivers the latest `admin_secret_rotated` package; after importing it, subsequent admin events validate under the updated key set. Central Command SHOULD rebroadcast the newest `admin_secret_rotated` package periodically throughout the overlap window so partitioned or late-joining nodes converge on the current secret quickly.
